...
import { Award, Banknote, ChevronRight, CreditCard, Headset, ShieldCheck, Sparkles, Star, Truck, Wallet } from 'lucide-react'
...
export function FeaturedProducts() {
  ...
  const featuredTags = ['Más vendida', 'Nueva llegada', 'Recomendada']
    ...
    <section className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredProducts.map((cuatrimoto, index) => (
            <ProductCard
              key={cuatrimoto.id}
              cuatrimoto={cuatrimoto}
              onVerMas={handleVerMas}
              onComprar={handleComprar}
              featuredTag={featuredTags[index]}
            />
          ))}
        </div>
        ...
        <div className="mt-12 text-center">
          <Link href="/catalogo">
            <Button size="lg" variant="outline" className="group hover-scale soft-shadow">
